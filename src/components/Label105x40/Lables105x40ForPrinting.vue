...
function textTo128(str) {
  ...
  var checksum = String.fromCharCode(
    modVal + 32 > 126 ? modVal + 32 + 18 : modVal + 32
  );
  ...
  return (
    String.fromCharCode(type128 + 100) +
    str +
    checksum +
    String.fromCharCode(typeClose + 100)
  );
}
</script>
<template>
  <div class="page-a4">
    <div
      ...
      class="label"
      alt="Kliknij na etykietę jeżeli chcesz ją usunąć"
      @click="$emit('removeLabel', label.id)"
    >
      <svg
        class="trash-icon"
        v-show="!trashIconIsVisible"
      ...
      <div class="top-part">
        <p id="index" class="index">{{ label.index.toUpperCase() }}</p>
        <p id="date" class="date">{{ label.created }} Made by Generator</p>
      </div>
      <h4 id="index-name" class="index-name">{{ label.name }}</h4>
      <div class="center-part">
        <p id="price" class="price">{{ label.price }}</p>
        <span id="unit" class="unit"
          ><b>{{ label.unit }}</b> w tym <b>{{ label.vat }}%</b> VAT</span
        >
      </div>
